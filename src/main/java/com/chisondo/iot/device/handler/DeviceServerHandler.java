...
/*
        ...
        log.debug("司机数量:" + driverAndDevice.size());
        log.debug("乘客数量:" + passengerAndDevice.size());
    ...
    //private void cancelOrder(Channel passenger, Document data) {
       /* String orderId = data.getString("orderId");
        ...
        if (size <= 0) {
            ...
            try {
                log.debug(Cmd.ORDER_LIST + orderId);
...
//                    if (jedis.llen(Cmd.ORDER_LIST + orderId) == 0) {
                        log.debug("订单已经被抢,不给司机乘客的信息,防止破解作弊!");
                        ...
                        log.debug("已经获得过订单;或者不在附近司机范围之内");
        ...
        watch.stop();
        log.debug("time:" + watch.getTotalTimeSeconds());
        watch.prettyPrint();*/
    //}
    ...
    //private void startOrder(Channel passenger, Document orderData, List<Document> driverList) {
        ...
        for (Document doc : driverList) {
            ...
            if (driverChannel == null) {
                ...
                log.debug("司机没有绑定,不能推送:" + doc.get("_id"));
...
//                > db.location.find( {position: { $near: [0,0], $maxDistance: 0.7  } } )
        ...
        log.debug("near driver count:" + list.size());
